...
export interface DriftWallProps {
  items?: DriftWallItem[];
  /** Pre-grouped columns — overrides `items` + `columns` round-robin when provided. */
  groupedItems?: DriftWallItem[][];
  columns?: number;
  tileWidth?: number;
  tileHeight?: number;
  gap?: number;
...
const DriftWall = ({
  ...
  const effectiveColumns = groupedItems ? groupedItems.length : columns;

  const columnItems = useMemo<DriftWallItem[][]>(() => {
    if (groupedItems) {
      return groupedItems.map(col => (col.length ? col : items.slice(0, 1)));
    }
  ...
  }, [items, groupedItems, effectiveColumns]);
  ...
  }, [columnItems, tileHeight, gap, containerHeight, effectiveColumns]);
  ...
  }, [columnItems, speed, direction, variance, effectiveColumns]);
  ...
  const imgClass = cx(
    'block h-full w-full select-none object-contain p-5',
  ...
  const tooltipClass = cx(
    'pointer-events-none absolute -bottom-2 left-1/2 z-50',
    'px-3 py-1.5 rounded-md bg-black/90 text-white text-xs font-semibold whitespace-nowrap shadow-xl border border-white/10',
    'opacity-0 [transform:translate3d(-50%,10px,0)]',
    'transition-all duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)]',
    'group-[.is-active]/tile:opacity-100 group-[.is-active]/tile:[transform:translate3d(-50%,0,calc(var(--dw-lift)+20px))]',
    'group-focus-visible/tile:opacity-100 group-focus-visible/tile:[transform:translate3d(-50%,0,calc(var(--dw-lift)+20px))]'
  );
